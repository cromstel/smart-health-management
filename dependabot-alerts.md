...
- glob CLI: command injection via `-c` (root dev tool) — **residual, dev-only**: no production path; tracked in local `docs/CHANGELOG.md`.
- js-yaml prototype pollution in `<<` (root dev tool, via eslint toolchain) — **residual, dev-only**: no production path; tracked in local `docs/CHANGELOG.md`.